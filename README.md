...
入力の色と透明度を保ったまま、素材の移動や反応、固体の変形と破断、燃焼、爆発、発光、影をシミュレーションできます。
...
シミュレーションでは57種類の物理素材を扱います。
`素材割り当て`を自動判定にすると、入力色を55色のパレットへ分類して対応する素材へ変換します。残り火と火薬は自動判定の対象外です。
単一素材を選ぶと、画像全体を指定した1種類の素材として扱います。
...
制御点爆発では、指定位置から円形の衝撃波が広がり、粉体や液体の飛散、固体への衝撃と破断、発光が連動します。
火薬へ着火した場合は、素材を伝わる圧力によって別の爆発が発生し、条件がそろうと連鎖爆発します。
燃え広がりは爆風による一括変換ではなく、火から周囲へ伝わる通常の素材反応として進みます。
...
既定の`環境光=100%`では、元の映像の明るさを保ったまま発光を加えます。
...
- 固体と流体の接触、影、爆発などは、リアルタイム映像エフェクト向けの近似です。
...
主要なシミュレーション処理はDirect3D 11のCompute Shader上で実行し、セル状態をCPUへ読み戻さずGPU上に保持します。
