...
YMM4-SandSimは、Direct3D 11上のセルオートマトン（CA）を基盤にしています。
固定素材までCPU側の剛体シミュレーションへ移すと、GPUとCPUの同期、状態の読み戻し、CAとは別の物理世界が必要になります。

そこで固定素材をGPU上のセル格子から切り離し、連続座標を持つ粒子として落下、回転、衝突、変形、破断させます。
CA、固体物理、爆発、照明の状態はGPU上で接続し、シミュレーション中のCPU readbackやblocking synchronizationは行いません。

## 設計方針

- 既存のセル格子をbroadphase、ownership、CA連携へ流用します。
- 固体はGPU Connected ComponentsでBodyへまとめ、XPBDで拘束を解きます。
- CAと固体は同じ論理グリッド上のoccupancyを介して連携します。
- 火薬爆発と制御点爆発は、見た目と時間スケールに合わせて別の伝播経路を使います。
- シークや巻き戻しでは不可逆なGPU状態を保持せず、現在入力から再構築します。

## ソルバー構成

- **表現**：1個の固体source cellを1個のGPU particleとして保持します。
  状態は`float4`で、現在位置を`xy`、直前位置を`zw`へ格納します。
- **Body同定**：GPU Connected Componentsで、同一素材・4近傍・未破断bondでつながるsource cell群へ同じBody IDを付けます。
  離れた同素材と異素材は別Bodyです。CPU body graphは使用しません。
- **トポロジ**：XPBDの候補edgeはsource imageの8近傍です。同じConnected Body IDを持つ端点だけをconstraintとして解きます。
- **積分**：damped Verletとgravityを使用し、substepは固定`1/120 s`です。
- **制約**：XPBD distance constraintを使用します。
  水平・垂直のrest lengthは1、斜めは`sqrt(2)`です。
- **スケジューリング**：2×2 blockを4種類のparity phaseで処理し、1 dispatch内の書き込み先を重複させません。
  水平辺と垂直辺では重複位相を除外し、斜辺を含む8近傍の各edgeをsolver反復あたり1回だけ解きます。
  論理サイズが奇数の場合も、最終行と最終列を偶数パディングセルと組み合わせて拘束対象にします。
- **衝突のbroadphase**：論理cell gridそのものを使用します。
  continuous positionをgridへ投影し、`InterlockedMin`で各cellのownerを決定します。
- **衝突応答**：occupancy競合を外部接触とBody内部競合に分離します。
  外部接触だけをBody代表へatomic ORで集約し、床ならY、壁ならXの速度成分だけを止めます。
  接線方向の運動量は残し、同じBody内部のセル競合でBody全体を停止させません。
- **空洞**：Body IDは固体source cellにだけ存在します。
  内部が`Empty`ならoccupancyやcolliderを作らないため、1つの巨大Bodyでも穴や通路を維持できます。
...
- occupancy競合はatomic minimumで解決し、処理順へ依存させません。
...
現在のモデルには、液体の体積押し出しと圧力、rigidが粉体を押し退ける処理、完全な流体・固体接触、任意位置での新規rigid particle poolは含めていません。
...
- **Density**：XPBD projectionで使う逆質量（inverse mass）へ変換します。
  重力加速度そのものは質量に依存しません。
...
同素材でも、空洞、切断、永続破断markerによって4近傍接続が途切れれば、次のBody再構築で別Body IDへ分離します。

制御点爆発の粗い破断格子は`ExplosionRadius`から独立させ、素材別の物理スケールを使用します。
ガラスと氷は小さく、石系は中程度、木や金属は大きな破片を保つ設定です。
...
破断したbondは、既存の`RigidLambda` textureへ大きな永続markerとして記録します。
...
Emitterのrestampでは既存のrigid source particleが持つ破断履歴を保持し、片側だけが再接着する状態を避けます。
...
爆発は、火薬と制御点で異なる伝播経路を使います。
燃焼はどちらの爆発経路からも分離し、既存のCA反応へ任せます。

### 火薬爆発

火薬のone-shot metadata eventを`SandExplosionUpdate`が消費し、GPU上のscalar pressure fieldを8近傍へ伝播します。
壁や素材の`ExplosionPressureTransmission`で圧力を減衰させ、火薬の連鎖、CAの飛散、XPBDへのpressure gradient、発光へ利用します。
...
- `SandRigidIntegrate`はpressure gradientをimpulseとして読みます。
  `RigidProperties`の密度を反映し、十分に鋭いimpulseはXPBD bondへ永続的な破断markerを設定できます。
...
- 着火直後のgunpowder cellは、pressure passがone-shot eventを消費するまでその位置へ固定します。
  CA reaction、movement、swapは、そのeventを持つcellを一時的に移動不可として扱います。

### 制御点爆発

YMM4の`VideoEffectController`から中心、発火フレーム、px単位の半径を受け取り、火薬pressureとは別の解析的Euclidean frontを最大半径まで進めます。
ホスト側では約6出力フレームで最大半径へ到達させるため、大半径でもセルを1個ずつ待つ遅い同心円伝播にはなりません。

制御点の中心はsigned cell座標としてGPUへ渡し、画面端へclampしません。
そのため、画面外の爆心から衝撃波の一部だけを画面内へ届かせることもできます。

同じConnected Bodyの全セルには、Body代表位置から求めた共通の放射方向へimpulseを加えます。
セルごとの速度差をXPBDが打ち消すのを抑え、Body全体が爆心から離れる方向へ動くようにします。
爆発後の外部接触は軸別に処理するため、床に触れても横方向の運動量を保持します。

強い制御点爆発は、素材別の破断スケールに従って永続bond markerを設定します。
Connected Componentsは次のBody再構築時に破断済み軸bondを接続として使わないため、同素材でも切断された島は別Bodyになります。

### 燃焼と発光

制御点爆発では、中心部を一度だけ`Empty`へ削り、空洞境界の薄いshellへ`Fire`をseedします。
木、油、硫黄、石炭などへの延焼は、爆風による直接変換ではなく通常のCA隣接反応で進みます。
XPBD固定素材も`SandRigidReact`から同じ化学反応を受け取ります。

火薬pressureと制御点の解析的frontは、どちらもrender-time light fieldをseedします。
暖色のshock ringとして波面を可視化し、flashと物理応答を同期します。

## タイムライン評価

Timeline evaluationは`Initial`、`Continuous`、`Random`へ明示的に分類します。

- **Initial**：現在の入力から状態を構築し、設定された上限内のwarm-upだけを適用します。
- **Continuous**：同一フレームの再評価と、次の連続フレームを含みます。
  同一フレームはパラメータ変更がなければzero-delta no-opとなり、次フレームだけ1回進めます。
- **Random**：シークと巻き戻しを含みます。
  不可逆なGPU状態を破棄し、その位置の現在入力から再構築します。

アイテム先頭から過去の全フレームを再生して履歴を再現する方式ではありません。
...
Connected Componentsは複数のunion/compress dispatchを必要とします。
各roundでshader、SRV、UAVを再bindせず、同じstateを保持してconstant bufferだけ更新します。
これによりGPUアルゴリズムを変えずに、CPU/driver側のstate changeを減らします。

`YMM4SANDSIM_LOG_LEVEL=Information`を明示した場合だけ、120回単位でsimulation/renderの`cpuSubmitMs`とCompute Shader dispatch数をログへ集計します。
`cpuSubmitMs`はCPUがD3D11コマンドを発行する時間であり、GPU実行時間ではありません。

設計ルールのGPU readback禁止を守るため、timestamp queryをCPUへ回収する内蔵GPU profilerは追加しません。
GPU時間を調べる場合は、PIXやRenderDocなどの外部ツールを使用します。

## GPUメモリの上限

Simulation bufferは最悪構成を常時確保せず、有効な機能に必要なbufferだけを確保します。
